...
const content = {
    hero: {
        title: 'Services',
        description: 'Let us help you unlock more value and impact from your digital services.'
    },
    approach: { ...approach, label: undefined, title: 'Our Approach' },
    capabilities: {
        title: 'Capabilities',
        description: 'We bring strong capabilities to modern digital services delivery.',
        items: [
            {
                title: 'Digital Modernization',
                description: 'We have deep expertise modernizing legacy systems by **building next versions in parallel** or **enhancing live systems in place**.',
                icon: FaCode,
                ctas: [
                    { title: 'See our modernization projects', path: '/our-work?filter=modernization' }
                ]
            },
            {
                title: 'AI & Emerging Technologies',
                description: "Our staff bring varied experience in **machine learning**, **large language models (LLMs)**, and **generative AI**.",
                icon: FaCode,
                ctas: [
                    { title: 'See AI assisted projects', path: '/our-work?filter=ai' }
                ]
            },
            {
                title: 'Security & ATO',
                ...
                icon: FaCode,
                ctas: [
                    { title: 'See ATO case studies', path: '/our-work?filter=ai' }
                ]
            },
            {
            ...
            <Box>
                {
                    content.services.servicesList.map((item: any) => {
                        return (
                            <Box mb={[12, 16]}>
                                <Heading fontSize={['2xl', '3xl']} fontWeight="semibold" mb={2}>{item.title}</Heading>
                                <Divider mb={[3, null, 5]} />
                                <SimpleGrid columns={[1, null, 2]} spacing={5}>
                                    <Box>
                                        <Text fontSize={['lg', 'xl']} mb={5}>{item.description}</Text>
                                        <Text fontSize={['md', 'lg']} color="teal" fontWeight="semibold" mb={[2, null, 3]}>Expertise</Text>
                                        <UnorderedList fontSize={['lg', 'xl']}>
                                            {item.expertise.map((exp: any) => (<ListItem>{exp}</ListItem>))}                                    
                                        </UnorderedList>
                                    </Box>
                                    <Box>
                                        <Text fontSize={['md', 'lg']} color="teal" fontWeight="semibold" mb={[2, null, 3]}>Approach</Text>
                                        <VStack width="100%">
